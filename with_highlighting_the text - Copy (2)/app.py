...
model = joblib.load(r"Models\plagiarism_model.pkl")

# Load the precomputed reference vectors
precomputed_reference_vectors = joblib.load(r"Models\precomputed_reference_vectors.pkl")

# Load the pretrained TF-IDF vectorizer
pretrained_tfidf_vectorizer = joblib.load(r"Models\tfidf_vectorizer.pkl")
...
# Specify the path to your stop words file
hindi_stopwords_file = r"Models\hindiST.txt"
telugu_stopwords_file = r"Models\telugu_stopwords.txt"

# Load Hindi and Telugu stop words
hindi_stopwords = load_stopwords_from_file(hindi_stopwords_file)
telugu_stopwords = load_stopwords_from_file(telugu_stopwords_file)

# Modify your preprocess_text function to use these stop words
...
def check_image_similarity():
    try:
        ...
        image_folder = r"Datasets\val"

        # Load the saved feature dictionary
        saved_features_path = r"Models\image_features_dict.pkl"  # Replace with the path to your saved feature dictionary
        ...
        print(f"The most similar image is {most_similar_image} with a cosine similarity of {highest_similarity:.2f}")

        # Build the paths for user-uploaded image and the most similar image
        image_folder_path = r"Datasets\val"
    ...
    try:
        ...
        if update_vectors_checkbox == "yes":
            if max_similarity < 0.5:
                ...
                joblib.dump(precomputed_reference_vectors,r"Models\precomputed_reference_vectors.pkl")
...
@app.route('/video_plagiarism.html')
def video_plagiarism():
    return render_template('video_plagiarism.html')
